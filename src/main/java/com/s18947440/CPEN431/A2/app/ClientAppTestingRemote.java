...
public class ClientAppTestingRemote {
    public static void main(String[] args) throws Exception {

        String serverHost = "localhost";
        int serverPort = 3102;

        RequestReplyClient client = new RequestReplyClient(serverHost, serverPort);
        ...
        */
        ...
        /*TEST1*/   /*
        RequestReplyServer server = new RequestReplyServer(43102);

        KeyValueRequest.KVRequest testReq =
                KeyValueRequest.KVRequest.newBuilder()
                        .setCommand(1)
                        .setKey(ByteString.copyFromUtf8("hello"))
                        .build();

        byte[] bytes = testReq.toByteArray();

        server.handleRequest(bytes);
        */
